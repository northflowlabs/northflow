      ...
      <div className="max-w-[1400px] mx-auto px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-16">
          <div>
            ...
            <p className="text-lg text-muted-foreground font-body leading-relaxed mb-6">
              EFOS is conceived as a unified AI-driven infrastructure that forecasts demand, generation, congestion, and pricing in the short term; orchestrates distributed assets across buildings, industry, storage, and mobility; provides real-time interfaces for DSOs, TSOs, and flexibility markets; implements EU-wide interoperability and data harmonisation; and ensures compliance with DES, EPBD, Data Act, AI Act, and NIS2. It is not a commercial software product but a long-term digital backbone intended to support EU system objectives.
            </p>
            <div className="space-y-4 mb-8">
              <div className="flex items-start space-x-3">
                <Icon name="CheckCircleIcon" size={20} variant="solid" className="text-primary mt-1 flex-shrink-0" />
                <div>
                  <p className="text-sm font-cta font-medium text-foreground mb-1">Architectural principles</p>
                  <p className="text-sm text-muted-foreground font-body">Framework design for operational independence and sovereignty principles.</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <Icon name="CheckCircleIcon" size={20} variant="solid" className="text-primary mt-1 flex-shrink-0" />
                <div>
                  <p className="text-sm font-cta font-medium text-foreground mb-1">Regulatory alignment pathways</p>
                  <p className="text-sm text-muted-foreground font-body">Compliance methodologies mapped to GDPR, NIS2, and institutional governance requirements.</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <Icon name="CheckCircleIcon" size={20} variant="solid" className="text-primary mt-1 flex-shrink-0" />
                <div>
                  <p className="text-sm font-cta font-medium text-foreground mb-1">Governance model development</p>
                  <p className="text-sm text-muted-foreground font-body">Structures enabling multi-stakeholder participation and framework evolution.</p>
                </div>
              </div>
            </div>
          ...
          <div
            ...
            className="bg-background border border-border rounded-sm p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-2 py-1 rounded-sm text-xs font-body ${
              milestone?.status === 'Current focus' ? 'bg-accent/10 text-accent' : 'bg-muted text-muted-foreground'}`
              }>
                  {milestone?.status}
                </span>
              </div>
