...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${SUPABASE_URL}/functions/v1/send-form-email`, {
      method: 'POST',
      headers: {
      ...
      return NextResponse.json(
        {
          ...
          message:
            result.error ||
            result.message ||
            'Failed to send form submission. Please try again or contact us directly at tdp@northflow.no',
        },
        { status: response.status }
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          'An error occurred while submitting your inquiry. Please try again or contact us directly at tdp@northflow.no',
      },
      { status: 500 }
    );
  }
